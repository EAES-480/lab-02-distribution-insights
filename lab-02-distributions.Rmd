...
- You may talk through ideas with classmates.
- **All code must be typed by you.**
- If you collaborated, add a note near the top:

> Collaborators (if any): _______________________________

## Grading rubric (lightweight)

- **Code correctness (40%)**: chunks run, outputs sensible, no silent failures
- **Interpretation quality (40%)**: answers are specific, distribution-aware, complete sentences
- **Reproducibility (20%)**: knits cleanly; no console-only objects; no hard-coded local paths
...
# 2) Set a seed so any randomness is reproducible
set.seed(___)
...
# 4. Warm-up: small coding tasks (momentum)

## 4.1 Create a working tibble

```{r warmup_df, echo=TRUE}
# GOAL: Create a clean working dataset named df.
# TODO: Remove unnecessary columns and keep only what you need.
...
- Suggested response variables (pick ONE): `dwdc_gm2`, `litterc_gm2`, `agunderstoryc_gm2`
- Suggested grouping variable: `foresttype` (categorical)

```{r choose_vars, echo=TRUE}
...
## 4.3 Filter missing values and create a z-score

```{r warmup_clean, echo=TRUE}
# GOAL: Keep complete cases and create a standardized version of your response variable.
# TODO: Fill in the blanks.

df2 <- df %>%
  filter(!is.na(.data[[response_var]])) %>%
  mutate(
    response_z = ( .data[[response_var]] - mean(.data[[response_var]], na.rm = TRUE) ) /
                 sd(.data[[response_var]], na.rm = TRUE)
  )

# CHECK: df2 should have the same columns as df plus response_z
glimpse(df2)
summary(df2$response_z)
```

**Prompt (1–2 sentences):** Why might z-scores be useful when comparing across variables or datasets?
...
# 8. Probability mini-module (optional, but aligned to lecture)
...
```{r extension_transform, echo=TRUE}
